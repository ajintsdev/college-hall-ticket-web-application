...
if (isset($_POST['submit-btn'])) {
  ...
  $check_query = "SELECT * FROM Subjects WHERE subject_name = '$subject_name' AND course_id = $course_id AND semester_id = $semester_id";
  $check_result = mysqli_query($conn, $check_query);

  if (mysqli_num_rows($check_result) > 0) {
      // Subject already exists
      echo "<script>alert('Subject already exists for this course and semester.');</script>";
  } else {
      // SQL query to insert data into the Subjects table
      $insert_query = "INSERT INTO Subjects (subject_name, course_id, semester_id) VALUES ('$subject_name', $course_id, $semester_id)";
...
<html lang="en">
  ...
  <ul>
    ...
    <main>
      <div class="container">
        ...
        <form action="addSubjects.php" method="post">
          ...
          <select id="course_id" name="course_id" selected>
        ...
        <?php endwhile; ?>
          ...
          <select id="semester_id" name="semester_id" selected>
          <option value="" disabled selected>Select a course</option>
    ...
    <h1 class="text-2xl font-bold mb-4">Subjects Table</h1>
    <table class="min-w-full bg-white border border-gray-200">
        <thead>
            <tr>
                <th class="py-2 px-4 border-b border-r">Subject</th>
                <th class="py-2 px-4 border-b border-r">Course</th>
        ...
        <tbody>
    <?php
    // Updated query with JOINs to fetch course_name and semester_name
    $query = "
    SELECT subjects.subject_id, subjects.subject_name, courses.course_name, semesters.semester_name
    FROM subjects
    JOIN courses ON subjects.course_id = courses.course_id
    JOIN semesters ON subjects.semester_id = semesters.semester_id
";
...
?>
    <tr>
        <td class="py-2 px-4 border-b border-r"><?php echo $row['subject_name']; ?></td>
        <td class="py-2 px-4 border-b border-r"><?php echo $row['course_name']; ?></td>
        <td class="py-2 px-4 border-b border-r"><?php echo $row['semester_name']; ?></td>
        <td class="py-2 px-4 border-b text-center">
            <a class="text-red-500 hover:text-red-700" href="delete_subject.php?id=<?php echo $row['subject_id']; ?>"> 
