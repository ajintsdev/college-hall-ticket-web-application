...
$sql = "SELECT * FROM exams"; // Assuming your table is named 'exams'
  ...
  <ul>
    ...
    <main>
      <div class="addExam container">
        <h1 class="text-3xl font-bold mx-auto text-center mb-4">Create Exam Table</h1>
        <form id="examForm" action="addExam.php" method="post">
          ...
          <input type="text" id="exam_name" name="exam_name" placeholder="e.g., First Internal Examinations October 2024 Semester 5 BCA" required>
        ...
        <?php endwhile; ?>
          ...
          <label for="start_date">Exam Start Date:</label>
          <input type="date" id="start_date" name="start_date" required>
      
          <label for="end_date">Exam End Date:</label>
          <input type="date" id="end_date" name="end_date" required>
             <!-- Submit Button -->
            <input type="submit" name="submit-btn" class="submit-btn" value="Create Exam" />
        </form>
      </div>
      <div class="container mx-auto">
    <h1 class="text-2xl font-bold mb-4">Exam Schedule</h1>
    <div class="overflow-x-auto">
        <table class="min-w-full bg-white border border-gray-200">
            <thead>
                <tr>
                    <th class="py-2 px-4 border-b border-r">Exam</th>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while($row = $result->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td class='py-2 px-4 border-b border-r'>" . htmlspecialchars($row["exam_name"]) . "</td>";
                        echo "<td class='py-2 px-4 border-b border-r'>" . htmlspecialchars($row["start_date"]) . "</td>";
                        echo "<td class='py-2 px-4 border-b border-r'>" . htmlspecialchars($row["end_date"]) . "</td>";
                        echo "<td class='py-2 px-4 border-b'><a href='delete_exam.php?exam_id=" . $row["exam_id"] . "' class='text-red-500 hover:text-red-700' onclick='return confirm(\"Are you sure you want to delete this Exam?\");'><i class='fas fa-trash-alt'></i> Delete</a></td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='4' class='py-2 px-4 border-b'>No exams found</td></tr>";
                }
                ?>
            </tbody>
        </table>
